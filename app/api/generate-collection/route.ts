...
- Link 2 (SECOND): Next collection → ${shopUrl}/collections/${nextCollection.handle}
  Anchor: "${nextCollection.name.toLowerCase()}"
...
- Link 1: Previous → ${shopUrl}/collections/${previousCollection.handle}
  Anchor: "${previousCollection.name.toLowerCase()}" (lowercase for natural flow)
- Link 2: Loop back to first → ${shopUrl}/collections/${allCollections[0].handle}
  Anchor: "${allCollections[0].name.toLowerCase()}" (lowercase for natural flow)
`;
    } else {
      linkInstructions = `
...
- Link 2: Next → ${shopUrl}/collections/${nextCollection.handle}
  Anchor: "${nextCollection.name.toLowerCase()}" (lowercase for natural flow)
`;
    }
...
H2 TITLES:
- Short (4-7 words max)
- Include main keyword "${collectionName}" in 3 out of 4 titles
- Address customer questions/needs
- If question format → add "?" at end
- Max 1-2 questions, rest statements
- One title can use synonym instead of exact keyword
...
STRONG TAGS (Minimal but Present):
- 1-3 per paragraph (aim for at least 1 per paragraph)
- Can be: single words OR short phrases (2-4 words max)
- Use for: materials, key benefits, differentiators, technical terms, niche-specific words, important info to emphasize
- Ask: "Would customer care?" If no → don't bold
- Examples: "or 18 carats", "livraison rapide", "fait main", "résistant à l'eau"
- Stay minimal, but identify the most relevant terms in each paragraph

INTERNAL LINKS (Exactly 2):
- Never say "collection" - use natural references
- Integrate in paragraphs 2 and 3
- Use EXACT anchor text provided (already in lowercase)
- CRITICAL: Keep anchors in LOWERCASE even in middle of sentence for grammatical correctness
- Example: "Découvrez notre boutique de grillz" NOT "Découvrez notre Boutique De Grillz"
...
- Start with <h2>`;
    ...
    const message = await anthropic.messages.create({
      ...
      max_tokens: 1500,
      system: `You are an expert SEO copywriter. Always respond in ${targetLang}. You write natural, engaging content with perfect internal linking strategy.`,
      messages: [
        {
          role: "user",
          content: prompt,
