...
import { Link2, Loader2, Download, Plus, Trash2, ArrowRight, Home, RefreshCw, Sparkles, AlertCircle, Layers, ChevronDown, Pause, Play, Upload } from "lucide-react";
...
export default function Collections() {
  ...
  const downloadShopifyFormat = () => {
    ...
    let output = "# CONTENU DES COLLECTIONS - FORMAT SHOPIFY\n";
    output += `# Boutique: ${cleanUrl}\n`;
    output += `# Date: ${new Date().toLocaleDateString()}\n`;
    output += `# Nombre de groupes: ${groups.length}\n\n`;
    output += "=".repeat(80) + "\n\n";

    groups.forEach((group, groupIndex) => {
      output += `\n${"#".repeat(80)}\n`;
      output += `# GROUPE ${groupIndex + 1}: ${group.name}\n`;
      output += `# ${group.collections.length} collection(s)\n`;
      output += `${"#".repeat(80)}\n\n`;

      group.collections.forEach((col, index) => {
        output += `COLLECTION ${index + 1}: ${col.name}\n`;
        output += `Handle: ${col.handle}\n`;
        output += `URL: ${cleanUrl}/collections/${col.handle}\n`;
        output += `Statut: ${col.status}\n`;
        if (col.linkCount) {
          output += `Liens internes: ${col.linkCount}\n`;
        }
        output += "\n--- CONTENU HTML (Copier dans Shopify) ---\n\n";
        output += col.content || "(Pas encore généré)";
        output += "\n\n" + "=".repeat(80) + "\n\n";
      });
    });
    ...
    link.setAttribute("download", `collections-shopify-${Date.now()}.txt`);
